...
% raster scan, sampled at the experimental bead positions.

function [FocalPts_RayModel_um,RayX,RayY] = find_FocalPts_BeadCoordinates(OP,Z,Scan,DC,Exp)
...
%% Bead positions at z = 0
cd(Exp.dataLocation);
load('xyCentre.mat','xCentre','yCentre');
numberBeads = length(xCentre);

for i_z = 1:length(Scan.Curvature)
    for bead = 1:numberBeads
        
        Exp.xBeadPosition = xCentre(bead);
        Exp.yBeadPosition = yCentre(bead);
        ...
        [start_aol,stop_aol,start_obj,stop_obj] = scanCoordinates_Experimental(OP,z_aol,Exp);
        ...
        FocalPts_RayModel_um(:,:,bead,i_z) = IntersectionPts_ray_um;
        FocalPts_RayModel_um(:,3,bead,i_z) = FocalPts_RayModel_um(:,3,bead,i_z)-Z.Z_normalised_zero*1e6;
    end
end
