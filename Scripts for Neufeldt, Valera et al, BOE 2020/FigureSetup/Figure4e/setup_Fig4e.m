function [OP,Scan,DC,Z,Exp] = setup_Fig4e(mainFolder)
...
Scan.DwellTime = 50e-9;          % Dwell time in s, single element or array
Scan.resolution = 512;           % Image pixel resolution
Scan.timePoints = 0;             % Time points at which to sample the modelled raster scan
Scan.Curvature = linspace(0.79,-0.91,81);             % Wavefront curvature in m^-1
Scan.Curvature(round(length(Scan.Curvature)/2)) = 0;  % Assymetric curvature range, set central value = 0
...
OP.xError = 0e-3;
OP.yError = 0e-3;
OP.zError = 60e-3;         OP.zError = OP.zError + (OP.zError==0)*eps;
...
Exp.dataLocation = [mainFolder,path_for_figures,'Figure4e/'];
Exp.fileName = 'Fig4e_xScan_f_minus750.xml';
Exp.numberPlanes = 71;
Exp.StepSize_microns = 400/(Exp.numberPlanes-1);
