function [OP,Scan,DC,Z,Exp] = setup_FigS10a(mainFolder)
...
OP.f_input_x = -7;               % x-AOL input beam curvature (non-linear distortion)
OP.f_input_y = -7;               % y-AOL input beam curvature (non-linear distortion)
OP.zOffset = -32.316e-6;         % z offset if input beam is not planar
...
Scan.ScanDirection = 'y';                       % 'x' or 'y' scan direction
Scan.DwellTime = 200e-9;                        % Dwell time in s, single element or array
Scan.resolution = 512;                          % Image pixel resolution
Scan.timePoints = linspace(-0.5,0.5,17);        % Time points at which to sample the modelled raster scan
Scan.Curvature = linspace(0.865,-0.865,41);     % Wavefront curvature in m^-1
...
Exp.dataLocation = [mainFolder,path_for_figures,'FigureS10a/'];
Exp.fileName = 'FigS10a_yScan_aa0_005_dt2e-07_1_Tracks.xml';
